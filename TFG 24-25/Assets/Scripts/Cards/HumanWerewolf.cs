...
public class HumanWerewolf : Character
{
    ...
    void ChangeForms()
    {
        ...
        if (isHumanForm)
        {
            ...
            DisplayActionsManager.Instance.CreateCustomText("=" + stats.dmg.ToString(), new Vector3(245, 0, 76), 8, 1.0f, token.transform.position);
            DisplayActionsManager.Instance.CreateCustomText("=" + stats.hp.ToString(), new Vector3(50, 245, 99), 8, 1.0f, token.transform.position, -0.2f);
            stats.PrintStats();
        }
        else
        {
